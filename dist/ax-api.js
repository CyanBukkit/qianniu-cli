...
exports.clickBuyer = clickBuyer;
exports.getChatMessages = getChatMessages;
exports.getChatMessagesFromTextArea = getChatMessagesFromTextArea;
...
function runScript(script) {
    ...
    try {
        return (0, child_process_1.execSync)(`osascript ${tmpFile}`, { timeout: 10000, encoding: 'utf8' }).trim();
    }
    finally {
        try {
 ...
 * 点击买家头像
 */
function clickBuyer(x, y) {
    (0, child_process_1.execSync)(`cliclick c:${x},${y}`, { timeout: 3000 });
  ...
  `;
    ...
    return texts.map(text => {
        ...
        if (cleanText.includes('客服:') || cleanText.includes('我:')) {
            return { sender: 'seller', text: cleanText };
        }
        return { sender: 'buyer', text: cleanText };
    });
}
/**
 * 获取买家的聊天消息（通过 AXTextArea）
 */
function getChatMessagesFromTextArea() {
    const script = `
    tell application "System Events"
      tell process "Aliworkbench"
        set out to ""
...
function sendMessage(text) {
    // 先复制到剪贴板
    (0, child_process_1.execSync)(`echo '${text.replace(/'/g, "\\'")}' | pbcopy`, { timeout: 3000 });
    // 聚焦到输入框并粘贴
    const script = `
    tell application "System Events"
      tell process "Aliworkbench"
