...
function parseCGWindowList(output: string): BuyerInfo[] {
  const buyers: BuyerInfo[] = [];
  
  if (!output || output.trim() === '') return buyers;
...
function parseReceptionList(output: string): {
  buyers: BuyerInfo[];
  windowBounds: { x: number; y: number; w: number; h: number } | null;
} {
  const buyers: BuyerInfo[] = [];
  let windowBounds: { x: number; y: number; w: number; h: number } | null = null;
...
function readChatByCGWindow(): ChatMessage[] {
  // 截图聊天区域，用 OCR 读文字
  ...
  try {
    ...
    execSync(`screencapture -R 280,100,900,600 ${tmpImg}`, { timeout: 5000 });
    
    // OCR
    const ocrResult = execSync(
      `tesseract ${tmpImg} stdout -l chi_sim+eng --psm 6 2>/dev/null`,
...
export function detect(): DetectionResult {
  ...
  try {
    ...
    cgWindows = execSync(`osascript ${tmp}`, { timeout: 8000, encoding: 'utf8' }).trim();
  ...
  try {
    ...
    receptionInfo = execSync(`osascript ${tmp}`, { timeout: 8000, encoding: 'utf8' }).trim();
    try { fs.unlinkSync(tmp); } catch {}
  } catch {
    receptionInfo = '';
  }
