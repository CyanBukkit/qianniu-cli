...
// ============ 截取指定窗口（类似 mac-screen-capture.captureWindow）============
 ...
 */
export function captureWindow(options: CaptureOptions): string {
  const {
    x = 0, y = 0, width = 0, height = 0,
    windowId,
    outputPath = `/Users/liuyuxuanyi/.qclaw/workspace/tmp/qianniu-msc-${Date.now()}.png`,
    format = 'png'
  } = options;
  ...
  if (windowId) {
    ...
    // 全屏
    execSync(`screencapture ${out}`, { timeout: 8000 });
  }
 ...
 * 列出所有窗口（类似 mac-screen-capture.listWindows()）
 */
export function listWindowsNative(): Array<{
  app: string;
  title: string;
  x: number;
  y: number;
  ...
  try {
    ...
    const output = execSync(`osascript -l JavaScript ${tmp}`, { timeout: 10000, encoding: 'utf8' }).trim();
...
export function detect(): DetectionResult {
  ...
  // 2. 找接待中心窗口
  const receptionWin = windows.find(w => 
    w.title.includes('接待中心') || w.title.includes('Aliworkbench')
  );
  ...
  try {
    ocrText = execSync(
      `tesseract ${listCapture} stdout -l chi_sim+eng --psm 6 2>/dev/null`,
  ...
  // 清理临时文件
  try { fs.unlinkSync(fullCapture); } catch {}
  try { fs.unlinkSync(listCapture); } catch {}
  ...
  if (newBuyer) {
    // 点击买家头像
    try {
      execSync(`cliclick c:${newBuyer.x + 60},${newBuyer.y + 20}`, { timeout: 3000 });
      execSync('sleep 0.8', { encoding: 'utf8' });
    } catch {}
    ...
    try {
      const chatOcr = execSync(
        `tesseract ${chatCapture} stdout -l chi_sim+eng --psm 6 2>/dev/null`,
        { timeout: 20000, encoding: 'utf8' }
      ).trim();
    ...
    try { fs.unlinkSync(chatCapture); } catch {}
  }
...
function parseBuyerList(ocrText: string, win: {
  ...
  for (const line of lines) {
    ...
    // 过滤纯数字/英文/网址
    if (/^[a-zA-Z0-9\\.\-_:]+$/.test(clean)) continue;
    if (clean.includes('http') || clean.includes('www') || clean.includes('.com')) continue;
    // 过滤时间类
