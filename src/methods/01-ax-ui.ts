...
function screenshotAndOCR(x: number, y: number, w: number, h: number): string {
  if (w <= 0 || h <= 0) return '';
  const tmp = path.join(TMP_DIR, `qianniu-ocr-${Date.now()}.png`);
  try {
    execSync(`screencapture -R ${x},${y},${w},${h} "${tmp}"`, { timeout: 5000 });
    const text = execSync(`tesseract "${tmp}" stdout -l chi_sim+eng --psm 6 2>/dev/null`, {
...
function clickBuyerAndReadChat(buyerX: number, buyerY: number): string {
  const script = `
    tell application "System Events"
      tell process "Aliworkbench"
        -- 关闭接待中心，打开与该买家的聊天窗口
  ...
  try {
    ...
    const result = execSync(`osascript ${tmp}`, { timeout: 10000, encoding: 'utf8' }).trim();
    try { fs.unlinkSync(tmp); } catch {}
    return result;
  } catch {
    return '';
...
function parseBuyerList(rawOutput: string): {
  windows: string[];
  buyers: BuyerInfo[];
} {
  ...
  for (const entry of entries) {
    ...
    if (!name || isNaN(coords[0])) continue;
...
function parseOCRBuyers(ocr: string): Array<{ name: string; nickname: string }> {
  const results: Array<{ name: string; nickname: string }> = [];
  ...
  for (const line of lines) {
    ...
    // 过滤纯数字账号（如 t_xxx）
    if (/^t_\d+/.test(clean)) continue;
    // 过滤短文本（可能是标签）
    if (clean.length < 3 || clean.length > 20) continue;
    // 过滤含网址/特殊字符
    if (clean.includes('http') || clean.includes('www')) continue;
...
function parseOCRMessages(ocr: string): ChatMessage[] {
...
export function detect(): DetectionResult {
  ...
  try {
    ...
    rawOutput = execSync(`osascript ${tmpFile}`, { timeout: 15000, encoding: 'utf8' }).trim();
  ...
  if (newBuyer) {
    // 点击买家（右侧区域，避免点到分隔线）
    try {
      execSync(`cliclick c:${newBuyer.x + 130},${newBuyer.y + 26}`, { timeout: 3000 });
      execSync('sleep 1.5', { encoding: 'utf8' });
    } catch {}
    ...
    try {
      ...
      axChat = execSync(`osascript ${tmp}`, { timeout: 8000, encoding: 'utf8' }).trim();
      try { fs.unlinkSync(tmp); } catch {}
    } catch {}
